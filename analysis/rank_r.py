...
# 文件路径
files = ['Zero Response.xlsx', 'One Response.xlsx', 'Three Response.xlsx']
...
for file in files:
    ...
    for column in ['Realistic', 'Deformation', 'Imagination', 'Color Richness', 'Color Contrast', 'Line Combination', 'Line Texture', 'Picture Organization', 'Transformation']:
        corr, _ = pearsonr(df_response[column], df_expert[column])
...
# 将结果保存到Excel文件
results.to_excel('Pearson Correlation Results.xlsx', index=False)
print("结果已保存到 'Pearson Correlation Results.xlsx'")
