...
model = AutoModelForCausalLM.from_pretrained("/home/owner/syf/Model/Qwen-VL-Chat-Int4", device_map="cuda",
                                             trust_remote_code=True).eval()

# 定义评估标准的字段
criteria = ["Realistic", "Deformation", "Imagination", "Color Richness", "Color Contrast", "Line Combination",
            "Line Texture", "Picture Organization", "Transformation"]
...
for i in range(1, 51):
    ...
    # 解析响应并存储结果
    scores = eval(response)  # 将字符串转换为字典
    scores['Image'] = image_filename
    results.append(scores)

# 将结果保存到Excel文件中
df = pd.DataFrame(results, columns=["Image"] + criteria)
df.to_excel("artwork_assessment.xlsx", index=False)
